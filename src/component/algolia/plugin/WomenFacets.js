  ...
  useEffect(() => {
    if (loading) {
      index
        .search("", {
          facetFilters: [
            "hierarchical_categories.lvl2: Women > Clothing > T-shirts",
          ],
          hitsPerPage: 6,
        })
        .then((results) => {
              ...
              <a href={`/p/${wm?.objectID}`}>
                <div>
                  <div>
                    <img
                      src={wm?.image_urls[0]}
                      className="md:aspect-[8/10] hover:scale-110 transition delay-300 duration-500 ease-in-out"
                      style={{ width: "100%" }}
                    />
                  </div>
                  <div className="hits_name">{wm?.name}</div>
                  <p className="hits_price">
                    {wm?.price?.currency + " " + wm?.price?.value.toFixed(2)}
                  </p>
                  <div
                    className="hit_color"
                    style={{ backgroundColor: wm?.color?.original_name }}
                  />
                </div>
              </a>
            </SwiperSlide>
